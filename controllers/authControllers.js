...
const register = async (req, res) => {
  ...
  const verifyEmail = {
    ...
    html: `<a target='_blank' href='${PROJECT_URL}/api/auth/verify/${verificationPass}'>Click verify email</a>`,
  };
...
const verify = async (req, res) => {
  const { verificationPass } = req.params;
  const user = await User.findOne({ verificationPass });
  if (!user) throw HttpError(404, "Email not found or already verified");

  await User.findByIdAndUpdate(
    { id: user._id },
    { verify: true, verificationPass: "" }
  );
